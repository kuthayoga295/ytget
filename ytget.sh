...
for cmd in yt-dlp ffmpeg awk; do
  if ! command -v $cmd &>/dev/null; then
    echo "❌ Dependensi '$cmd' tidak ditemukan! Silakan install terlebih dahulu."
    exit 1
  fi
done
...
# === Input URL ===
echo "=== YouTube Downloader dengan Filter Codec ==="
...
# === Tampilkan format ===
echo ""
echo "=== Format Video dengan Codec '${codec_filter:-Semua}' ==="
i=1
